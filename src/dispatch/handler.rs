...
pub trait Handler {
    ...
    /// # 产出什么
    ///
    /// 它产出的东西要能待在 `<p>` 里面。渲染器会照看这件事：在行内上下文里
    /// [`NodeKind::Instance`](crate::NodeKind::Instance) 会渲染成 `<span>` 而不是
    /// `<div>`，报错也会渲染成行内 `<span>`。**但
    /// [`NodeKind::Element`](crate::NodeKind::Element) 是个例外**——标签是你自己
    /// 写的，所以往里塞 `div`/`h2` 这类块级标签就是你的责任了。
    ///
    /// 另外注意：**只实现了 [`expand_call`](Handler::expand_call) 的展开器不会
    /// 被行内调用用到**——调度器按种类分派，行内走的是这个方法，没实现就落到
    /// 默认的报错上。所以块级包装器绝不会意外地把 `<div>` 漏进段落里。
    ///
